...
** Last update Thu Jun  5 16:35:28 2014 sainto_p
*/
...
int     calc(t_img *img, int i, int j)
{
  ...
  k[5] = calc_second_sphere(vec);
  k[6] = calc_tri_sphere(vec);
  k[7] = lum_sphere(vec);
  if (k[0] >= PLAN && ((k[1] >= PLAN && k[0] <= k[1]) || k[1] <= 0))
    ...
    return (0xFFCC99);
  else if (k[5] >= PLAN && ((k[1] >= PLAN && k[5] <= k[1]) || k[1] <= PLAN))
    return (0x8B6C42);
  else if (k[6] >= PLAN && ((k[1] >= PLAN && k[6] <= k[1]) || k[1] <= PLAN))
    return (0x8B6C42);
  else if (k[1] > PLAN)
    return (0x8B6C42);
  return (0);
